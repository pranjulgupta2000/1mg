...
export default function Nutrition({ navigation }) {
    ...
    const [filterData, NutritionFiltersData] = useState([]);

    const hideModal = () => {
        setModalVisible(false);
    };
    const lowToHigh = () => {
        nutrition.sort(function (a, b) {
    ...
    const fetchNutrition = () => {
        setIsLoading(true);
        database.ref('nutrition').once('value').then((item) => {
            let users = [];
            item.forEach((childSnapshot) => {
                users.push(childSnapshot.val())
            })
            setNutrition(users); // updating stateMessages
            setIsLoading(false);
        });
    }
    ...
    const searchnutrition = (textToSearch) => {
        NutritionFiltersData(nutrition.filter(item => item.name.toUpperCase().includes(textToSearch.toUpperCase())));
    }
    ...
    return (
        <View style={{ flex: 1 }}>
            ...
            {isLoading ?
                ...
                <FlatList
                    data={filterData.length == 0 ? (input != "" ? [] : nutrition) : filterData}
