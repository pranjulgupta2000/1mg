...
export default function Ayurveda({ navigation }) {
    const [ayurveda, setAyurveda] = useState([]);
    const [isLoading, setIsLoading] = useState(false);
    const [modalVisible, setModalVisible] = useState(false);
    const [input, setInput] = useState("");
    const [filterData, AyurvedaFiltersData] = useState([]);

    const hideModal = () => {
        setModalVisible(false);
    };
    const lowToHigh = () => {
        ayurveda.sort(function (a, b) {
    ...
    const fetchAyurveda = () => {
        setIsLoading(true);
        database.ref('ayurveda').once('value').then((item) => {
            let users = [];
            item.forEach((childSnapshot) => {
                users.push(childSnapshot.val())
            })
            setAyurveda(users); // updating stateMessages
            setIsLoading(false);
        });
    }
    ...
    const searchayurveda = (textToSearch) => {
        AyurvedaFiltersData(ayurveda.filter(item => item.name.toUpperCase().includes(textToSearch.toUpperCase())));
    }
    ...
    return (
        <View style={{ flex: 1 }}>
            ...
            {isLoading ?
                ...
                <FlatList
                    data={filterData.length == 0 ? (input != "" ? [] : ayurveda) : filterData}
