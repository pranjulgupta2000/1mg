...
export default function PainKiller({ navigation }) {
    const [painKiller, setPainKiller] = useState([]);
    const [isLoading, setIsLoading] = useState(false);
    const [modalVisible, setModalVisible] = useState(false);
    const [input, setInput] = useState("");
    const [filterData, painKillerFiltersData] = useState([]);

    const hideModal = () => {
        setModalVisible(false);
    };
    const lowToHigh = () => {
        painKiller.sort(function (a, b) {
    ...
    const fetchPainKiller = () => {
        setIsLoading(true);
        database.ref('painKiller').once('value').then((item) => {
            let users = [];
            item.forEach((childSnapshot) => {
                users.push(childSnapshot.val())
            })
            setPainKiller(users); // updating stateMessages
            setIsLoading(false);
        });
    }
    ...
    const searchpainKiller = (textToSearch) => {
        painKillerFiltersData(painKiller.filter(item => item.name.toUpperCase().includes(textToSearch.toUpperCase())));
    }
    ...
    return (
        <View style={{ flex: 1 }}>
            ...
            {isLoading ?
                ...
                <FlatList
                    data={filterData.length == 0 ? (input != "" ? [] : painKiller) : filterData}
