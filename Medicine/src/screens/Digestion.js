...
export default function Digestion({ navigation }) {
    const [digestion, setDigestion] = useState([]);
    const [isLoading, setIsLoading] = useState(false);
    const [modalVisible, setModalVisible] = useState(false);
    const [input, setInput] = useState("");
    const [filterData, digestionFiltersData] = useState([]);

    const hideModal = () => {
        setModalVisible(false);
    };
    const lowToHigh = () => {
        digestion.sort(function (a, b) {
    ...
    const fetchDigestion = () => {
        setIsLoading(true);
        database.ref('digestion').once('value').then((item) => {
            let users = [];
            item.forEach((childSnapshot) => {
                users.push(childSnapshot.val())
            })
            setDigestion(users); // updating stateMessages
            setIsLoading(false);
        });
    }
    ...
    const searchDigestion = (textToSearch) => {
        digestionFiltersData(digestion.filter(item => item.name.toUpperCase().includes(textToSearch.toUpperCase())));
    }
    ...
    return (
        <View style={{ flex: 1 }}>
            ...
            {isLoading ?
                ...
                <FlatList
                    data={filterData.length == 0 ? (input != "" ? [] : digestion) : filterData}
                    // data={digestion}
