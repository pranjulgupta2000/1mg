...
export default function EyeDrops({ navigation }) {
    const [eyeDrops, setEyeDrops] = useState([]);
    const [isLoading, setIsLoading] = useState(false);
    const [modalVisible, setModalVisible] = useState(false);
    const [input, setInput] = useState("");
    const [filterData, EyeDropsFiltersData] = useState([]);

    const hideModal = () => {
        setModalVisible(false);
    };
    const lowToHigh = () => {
        eyeDrops.sort(function (a, b) {
    ...
    const fetchEyeDrops = () => {
        setIsLoading(true);
        database.ref('eyeDrops').once('value').then((item) => {
            let users = [];
            item.forEach((childSnapshot) => {
                users.push(childSnapshot.val())
            })
            setEyeDrops(users); // updating stateMessages
            setIsLoading(false);
        });
    }
    ...
    const searcheyeDrops = (textToSearch) => {
        EyeDropsFiltersData(eyeDrops.filter(item => item.name.toUpperCase().includes(textToSearch.toUpperCase())));
    }
    ...
    return (
        <View style={{ flex: 1 }}>
            ...
            {isLoading ?
                ...
                <FlatList
                    data={filterData.length == 0 ? (input != "" ? [] : eyeDrops) : filterData}
